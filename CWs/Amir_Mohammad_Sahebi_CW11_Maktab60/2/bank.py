...
class Account:
    ...
    def deposit(self):
        ...
        self.transactions.append(
            {"type": "deposit", "amount": add, "date&time": datetime.datetime.today().strftime("%m/%d/%y,%H:%M")})
    ...
    def withdraw(self, reduce):
        if self.check_inventory(reduce):
            ...
            # appending transactions' details to the list
            self.transactions.append(
                {"type": "withdraw", "amount": reduce,
                 "date&time": datetime.datetime.today().strftime("%m/%d/%y,%H:%M")})
            return True

    # defining a method for transfer to another account
    def transfer(self, account, move):
        if self.check_inventory(move):
            self.inventory -= move
            account.inventory += move
            # appending transactions' details to the list
            self.transactions.append({"type": "transfer", "amount": move, "destination account": account,
                                      "date&time": datetime.datetime.today().strftime("%m/%d/%y,%H:%M")})
            return True
